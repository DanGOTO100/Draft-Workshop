...
* [How Envelope Encryption works in practice](https://github.com/DanGOTO100/Draft-AWS-KMS-Workshop/blob/master/Section-2-Encryption-with-AWS-KMS.md#how-envelope-encryption-works-in-practice)
* [Envelope encryption. Server Side Encryption](https://github.com/DanGOTO100/Draft-AWS-KMS-Workshop/blob/master/Section-2-Encryption-with-AWS-KMS.md#envelope-encryption-server-side-encryption)
* [Envelope encryption. Client Side Encryption](https://github.com/DanGOTO100/Draft-AWS-KMS-Workshop/blob/master/Section-2-Encryption-with-AWS-KMS.md#envelope-encryption-client-side-encryption)
* [Direct Encryption with AWS KMS](https://github.com/DanGOTO100/Draft-AWS-KMS-Workshop/blob/master/Section-2-Encryption-with-AWS-KMS.md#encryption-using-aws-kms-with-no-data-key)
...
The process is the same as we have seen twice in the previos section of the workshop, please go back to it if you need to in the following link: "[Create the import material and encrypt it for the import](https://github.com/DanGOTO100/Draft-AWS-KMS-Workshop/blob/master/Section-1-Operating-with-AWS-KMS.md#step-3---create-the-import-material-and-encrypt-it-for-the-import)"
...
In the "Write" section, select "**GenerateDataKey**" operation. Aditionally select "**Encrypt**", "**Decrypt**" and the tagging operations **as you can see in image below**, we will need them for the nexts steps. and "**Any**" as resource. 
...
Click on "**Review Policy**" and then give the policy a name, for example "**KMSWorkshop-AditionalPermissions**".
...
* A CiphetextBlob which is the encrypted data key generated, in base64 enconding. 
...
The data to encrypted and decrypted by your applications would use envelope encryption in a similar way. It is a powerful mechanism, enabled by AWS KMS CMKs, to protect in two tiers, the integrity and confidentiality of your data.
...
You have completed the second section of the workshop. In the [next section](https://github.com/DanGOTO100/Draft-AWS-KMS-Workshop/blob/master/Section-3-Working-with-Web-App.md) we will work with a real-life Web App and will try to implement best practices.
